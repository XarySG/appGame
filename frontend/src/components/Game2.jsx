  ...
  const handleDeath = () => {
    ...
    fetch('http://localhost:3000/game2', {
      ...
      .then((response) => {
        ...
        if (response.ok) {
          console.log('Succesfully sent Tamagotchi Data.');
        } else {
          console.log('Error at sending Tamagotchi Data.');
        }
      })
      .catch((error) => {
  ...
  return (
    <div className="main">
      <div className="screen">
        ...
        <div className="stats">
          <p>Age: {age}</p>
          <p className="mt-4">Birth date: {date}</p>
        ...
        {health === 0 || happiness === 0 || hunger === 0 || energy === 0 ? (
          <div>
            <p>Your Tamagotchi has died entre terrible sufrimiento.</p>
          </div>
        ) : (
